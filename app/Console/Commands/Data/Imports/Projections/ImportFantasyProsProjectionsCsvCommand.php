...
class ImportFantasyProsProjectionsCsvCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'import:projections:fantasy-pros-csv';
    ...
    {
        $this->year = select('Select a year', [2025, 2024], 2025);
        $this->week = select('Select a week', range(1, 18), 4);
    ...
    public function getFileOpts(): array
    {
        $fileOpts = [];

        $baseDir = storage_path('data/rankings/fantasy-pros');
        ...
        foreach ($dirs as $dir => $label) {
            $files = $this->getFiles($baseDir . '/' . $dir);

            foreach ($files as $file) {
                $label = $dirs[$dir] . ' - ' . $file;
                $value = $baseDir . '/' . $dir . '/' . $file;

                $fileOpts[$value] = $label;
            }
        }
    ...
    public function getFiles(string $dir)
    {
        ...
        foreach ($rawFiles as $file) {
            if (! str_ends_with(Str::lower($file), '.csv')) {
                continue;
            }

            $regex = '/FantasyPros_' . $this->year . '_Week_' . $this->week . '_[A-Z]{1,2}_Rankings.csv/';
    ...
    public function getTeam(string $abb)
    {
        ...
        $teamAbb = match($abb) {
            'JAC' => NFLTeams::JAX,
            'WAS' => NFLTeams::WSH,
            default => NFLTeams::from($abb),
        };

        $team = Team::find($teamAbb->value);
