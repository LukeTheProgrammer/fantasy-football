...
import { type Player, type Position, type Team } from '@/types/models';
...
export default function NflPlayers({ players, teams, positions }: NflPlayersProps) {
  ...
  const positionOrder = useMemo(() => ['QB', 'RB', 'WR', 'TE', 'DST', 'K', 'OT', 'G', 'C', 'DT', 'DE', 'LB', 'CB', 'S', 'FB', 'LS', 'P'], []);
  ...
  const handleSavePlayer = async () => {
    ...
    try {
      ...
      setPlayersData((prevPlayers) => prevPlayers.map((player) => (player.id === updatedPlayer.id ? updatedPlayer : player)));
  ...
  const filteredPlayers = useMemo(() => {
    ...
    if (selectedPositions.length > 0) {
      filtered = filtered.filter((player) => player.position && selectedPositions.includes(player.position.id));
    }
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="flex-1 p-8">
        <Heading title="NFL Players" description="View and manage NFL players" />
          ...
          <Tabs defaultValue="players">
            <div className="mb-6 flex items-center justify-between">
              <div className="flex items-center">
                <TabsList>
                  <TabsTrigger className="w-[7rem]" value="players">
                    Players
                  </TabsTrigger>
                  <TabsTrigger className="w-[7rem]" value="aliases">
                    Aliases
                  </TabsTrigger>
                  <TabsTrigger className="w-[7rem]" value="missing">
                    Missing
                  </TabsTrigger>
                </TabsList>
              </div>
            ...
            <TabsContent value="players">
              <div className="mb-6 flex items-center justify-between gap-6">
                <div className="flex grow-1 items-center justify-start gap-6">
        ...
        <FormDialog
          ...
          isLoading={isSaving}
        >
          <PlayerForm formData={editForm} onChange={setEditForm} />
        </FormDialog>
      </div>
    </AppLayout>
  );
